...
export class AddbankPage extends Base{

    public bankType : string = '';
    public cardNo:string = '';
    public prePhone:string = '';
    constructor
    (
    ...
    private _check():boolean
    {
        if(this.bankType == '')
        {
            this.errorLog(this.toastController,'请选择开户银行');
            return false;
        }
        if(this.cardNo =='')
        {
            this.errorLog(this.toastController,'请正确输入银行卡号');
            return false;
        }
        if(this.checkMobile(this.prePhone) == false)
        {
            this.errorLog(this.toastController,'请正确输入手机号码');
            return false;
        }
        return true;
    }
    public blurInput()
    {
        let bankobj = this.checkBankName(this.cardNo);
        this.bankType = bankobj['bankName'];
    }
    public checkBankName(bankCard)
    {
      var cardTypeMap = {
        DC: "储蓄卡",
        CC: "信用卡",
    ...
    public addBank():void
    {
        if(this._check() === true)
        {
            this.C.postApi('home/indexA/getconf',{
                ...
                'pre_phone':this.prePhone
            }).subscribe(
                data=>{
                    if(data.result_code == 'SUCCESS')
                    {
                        console.log(data);
                        this.errorLog(this.toastController,'请输入您收到的短信码');
                        this.checkBank(data.sdMsgNo);
                    }
                    else{
            ...
            },
            {
                ...
                handler: data3 =>
                {
                    if(data3.msgcode == '')
                    {
                        ...
                        this.C.postApi('home/indexA/getconf',{
                          'method':'add_bankcard_smsconfirm',
                          'token':this.LS.getItem('TOKEN'),
                          'sdMsgNo':msgNo,
                          ...
                          data=>{
                            if(data.result_code == 'SUCCESS')
                            {
                              this.errorLog(this.toastController,'添加成功');
                              this.navCtrl.pop();
                              //this.checkBank(data.sdMsgNo);
                            }
                            else{
